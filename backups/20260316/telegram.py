...
class TelegramNotifier:
    ...
    def send_trade_closed(self, trade: Dict) -> bool:
        ...
        FIXED: Field name mapping — paper_trader sends pnl_usd/pnl_percent,
        not net_pnl/pnl_pct. Now checks ALL possible key names.
        """
        if not self.send_signals_flag:
            return False
        ...
        try:
            ...
            # ── Direction handling (int or string) ──
            direction = trade.get("direction", 0)
            if isinstance(direction, str):
                sig = direction if direction in ("LONG", "SHORT") else "SHORT"
            ...
            # ══ FIX: Check ALL possible field names for PnL ══
            net_pnl = float(
                trade.get("pnl_usd",           # paper_trader sends this
                trade.get("net_pnl",            # some code paths use this
                trade.get("net_pnl_usd", 0)))   # legacy name
            )

            pnl_pct = float(
                trade.get("pnl_percent",        # paper_trader sends this
                trade.get("pnl_pct", 0))        # some code paths use this
            )

            commission = float(
                trade.get("commission",
                trade.get("commission_usd", 0))
            )

            # ══ FIX: Recalculate PnL if still zero but prices available ══
            if abs(net_pnl) < 0.001 and entry > 0 and exit_p > 0:
                pos_size = float(trade.get("position_size_usd", 0))
                if pos_size > 0:
                    price_change = (exit_p - entry) / entry * direction
                    net_pnl = pos_size * price_change - commission
                    pnl_pct = price_change * 100
                    logger.info(
                        f"Recalculated PnL for notification: "
            ...
            # Show position size if available
            pos_size = float(trade.get("position_size_usd", 0))
            if pos_size > 0:
                lines.append(f"📦 Size: ${pos_size:,.0f}")
