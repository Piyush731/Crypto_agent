...
class TelegramCommandBot:
    ...
    def _register_commands(self):
        ...
        try:
            commands = [
                {"command": "status", "description": "Capital, positions, P&L overview"},
                {"command": "positions", "description": "Open positions with live prices"},
                {"command": "trades", "description": "Recent closed trades"},
                {"command": "balance", "description": "Capital & P&L breakdown"},
                {"command": "stops", "description": "Trailing stop levels"},
                {"command": "signals", "description": "Recent generated signals"},
                {"command": "performance", "description": "Win rate & stats"},
                {"command": "prices", "description": "Market prices & Fear/Greed"},
            ...
            if resp.status_code == 200 and resp.json().get("ok"):
                ...
                logger.warning(f"setMyCommands failed: {resp.text[:200]}")

        except Exception as e:
            logger.debug(f"Command registration: {e}")
    ...
    def _cmd_help(self) -> str:
        ...
        return (
            ...
            "  /stops — Trailing stop levels\n"
            "\n"
            "💡 <b>Shortcuts:</b> /pos /bal /perf /trail\n"
            "\n"
    ...
    def _cmd_stops(self) -> str:
        """Show trailing stop levels for open positions."""
        try:
            ...
            if not open_trades:
                return "📐 <b>No open positions</b>\n\nNo trailing stops to show."

            lines = [f"📐 <b>TRAILING STOPS</b> ({len(open_trades)})", ""]
            ...
            for t in open_trades:
                ...
                original_sl = float(t.get("stop_loss", 0))
                ...
                if notes_str:
                    try:
                        saved = _json.loads(notes_str)
                        if isinstance(saved, dict):
                            current_stop = float(saved.get("current_stop", original_sl))
                            highest = float(saved.get("highest_price", entry))
                            lowest = float(saved.get("lowest_price", entry))
                    except Exception:
                        pass
                ...
                # Calculate profits
                if is_long:
                    profit_at_stop = (current_stop - entry) / entry * 100 if entry > 0 else 0
                    peak_profit = (highest - entry) / entry * 100 if entry > 0 else 0
                    current_profit = (current - entry) / entry * 100 if entry > 0 and current > 0 else 0
                else:
                    profit_at_stop = (entry - current_stop) / entry * 100 if entry > 0 else 0
                    peak_profit = (entry - lowest) / entry * 100 if entry > 0 else 0
                    current_profit = (entry - current) / entry * 100 if entry > 0 and current > 0 else 0

                trail_moved = abs(current_stop - original_sl) > 0.01
                ...
                if current > 0:
                    lines.append(f"  📍 Now: <code>${current:,.2f}</code> ({current_profit:+.2f}%)")

                lines.append(f"  🛑 Original SL: <code>${original_sl:,.2f}</code>")

                if trail_moved:
                    lock_emoji = "🔒" if profit_at_stop > 0 else "📍"
                    lines.append(f"  {lock_emoji} Trail Stop: <code>${current_stop:,.2f}</code>")
                    if profit_at_stop > 0:
                        lines.append(f"  ✅ If hit: locks <code>+{profit_at_stop:.2f}%</code>")
                    else:
                        lines.append(f"  ⚠️ If hit: <code>{profit_at_stop:+.2f}%</code>")
                else:
                    lines.append(f"  📍 Trail: at original SL (not moved yet)")

                lines.append(f"  📈 Peak: <code>+{peak_profit:.2f}%</code>")
                lines.append(f"  🎯 TP: <code>${tp:,.2f}</code>")
                lines.append("")
            ...
            logger.error(f"Stops command error: {e}")
            return f"❌ Stops error: {e}"
