...
class EnsemblePredictor:
    ...
    def predict(self, X: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            raw_prob_up = wp_up / wt

            # ══ CALIBRATION FIX: Tree models are overconfident ══
            # Extreme raw probabilities (< 0.15 or > 0.85) are usually
            # the model memorizing training data, not genuine edge.
            # Apply extra dampening for extreme values.
            if raw_prob_up < 0.15 or raw_prob_up > 0.85:
                # Aggressive pull toward 0.5 for extreme confidence
                prob_up = 0.5 + (raw_prob_up - 0.5) * 0.35
                logger.info(
                    f"    ⚠️ Extreme raw P(UP)={raw_prob_up:.3f} → "
                    f"dampened to {prob_up:.3f} (0.35x)"
                )
            else:
                prob_up = 0.5 + (raw_prob_up - 0.5) * self._prob_dampen
            ...
            # ══ SUSPICION CHECK: All models agreeing strongly = possible overfit ══
            if agreement >= 1.0 and (raw_prob_up < 0.12 or raw_prob_up > 0.88):
                logger.warning(
                    f"    ⚠️ ALL models unanimously extreme "
                    f"(raw={raw_prob_up:.3f}) — possible overfit, "
                    f"reducing confidence"
                )
                confidence = confidence * 0.7  # Penalize suspiciously unanimous extreme
    ...
    def predict_batch(self, X: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            raw_p_up = w_proba[:, 1].copy()

            # Calibration: extra dampening for extreme probabilities
            p_up = np.where(
                (raw_p_up < 0.15) | (raw_p_up > 0.85),
                0.5 + (raw_p_up - 0.5) * 0.35,      # Aggressive for extremes
                0.5 + (raw_p_up - 0.5) * self._prob_dampen  # Normal
            )
            p_dn = 1.0 - p_up
