...
class PaperTrader:
    ...
    def _check_open_positions(self) -> List[Dict]:
        ...
        for trade in open_trades:
            try:
                ...
                old_stop_val = state["current_stop"]
                state["current_stop"] = exit_check.get(
                    "new_trailing_stop", state["current_stop"]
                )

                # Notify if trailing stop just crossed breakeven (locks profit)
                new_stop_val = state["current_stop"]
                if abs(new_stop_val - old_stop_val) > 1e-8:
                    if direction == 1:
                        was_loss = old_stop_val <= entry_price
                        now_profit = new_stop_val > entry_price
                    else:
                        was_loss = old_stop_val >= entry_price
                        now_profit = new_stop_val < entry_price

                    if was_loss and now_profit:
                        locked_pct = abs(new_stop_val - entry_price) / entry_price * 100
                        sig_lbl = "LONG" if direction == 1 else "SHORT"
                        logger.info(
                            f"🔒 [{symbol}] Trail locked profit! "
                            f"stop=${new_stop_val:.2f} locks {locked_pct:.2f}%"
                        )
                        try:
                            from notifications.telegram import get_notifier
                            tg = get_notifier()
                            tg.send_message(
                                ...
                                f"<b>{sig_lbl} {symbol}</b>\n"
                                f"📍 Trailing stop passed breakeven\n"
                                f"💰 Entry: <code>${entry_price:,.2f}</code>\n"
                                f"🛑 Trail: <code>${new_stop_val:,.2f}</code>\n"
                                f"🔒 Locks: <code>+{locked_pct:.2f}%</code> profit"
                            )
                        except Exception:
                            pass
    ...
    # ═══════════════════════════════════════════════════════════
    #  CORRELATION GROUP CHECK — prevents 3 identical positions
    # ═══════════════════════════════════════════════════════════

    def _check_correlation_allowed(self, symbol: str, direction: int) -> bool:
        """
        Check if opening this position would violate correlation group limits.
        
        BTC/ETH/SOL are 90%+ correlated — opening all 3 SHORT is basically
        6x leverage on one bet. This limits same-direction positions per group.
        """
        corr_groups = RISK_CONFIG.get("correlation_groups", {})
        max_same = RISK_CONFIG.get("max_same_direction_per_group", 1)

        if not corr_groups:
            return True  # No groups configured
        ...
        try:
            ...
            for group_name, group_symbols in corr_groups.items():
                ...
                # Count open positions in same direction in this group
                same_dir_count = 0
                same_dir_symbols = []
                for t in open_trades:
                    t_sym = t.get("symbol", "")
                    t_dir = int(t.get("direction", 0))
                    if t_sym in group_symbols and t_dir == direction:
                        same_dir_count += 1
                        same_dir_symbols.append(t_sym)

                if same_dir_count >= max_same:
                    dir_label = "LONG" if direction == 1 else "SHORT"
                    logger.info(
                        f"[{symbol}] BLOCKED by correlation: "
                        f"{same_dir_count} {dir_label} already open in "
                        f"'{group_name}' group {same_dir_symbols} "
                        f"(max {max_same} per group)"
                    )
                    return False
    ...
    def _process_symbol(self, symbol: str) -> Dict:
        ...
        FIXED v3.1:
          - Defense-in-depth confidence check (belt + suspenders)
          - Correlation group check before opening
        """
        ...
        try:
            ...
            # ══ NEW: Defense-in-depth confidence check ══
            # Don't rely solely on risk_manager — check here too
            min_conf = RISK_CONFIG.get("min_confidence_to_trade", 0.55)
            if confidence < min_conf:
                result["rejected_reason"] = f"confidence {confidence:.3f} < {min_conf}"
                logger.info(
                    f"[{symbol}] Rejected (pre-filter): "
                    f"confidence {confidence:.3f} < {min_conf}"
                )
                return result

            # ══ NEW: Correlation group check ══
            direction = int(signal.get("direction", 0))
            if not self._check_correlation_allowed(symbol, direction):
                result["rejected_reason"] = "correlation_group_limit"
                logger.info(f"[{symbol}] Rejected: correlation group limit")
                return result
        ...
        try:
            ...
            try:
                ...
                tg.send_trade_closed({
                    "symbol": symbol,
                    "direction": direction, #was sig_label 14march update
                    "entry_price": entry_price,
