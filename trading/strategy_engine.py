"""Adapter from pluggable strategy outputs to the common v4 engine."""
...
class StrategyBacktestEngine:
    @staticmethod
    def _config(strategy: StrategyPlugin, starting_capital: float):
        policy = strategy.policy
        return V4ExecutionConfig(
    ...
    def run_signals(
        ...
        execution_features = features.copy()
        if "atr_pct" in signals.columns:
            execution_features.loc[signals.index, "atr_pct"] = signals["atr_pct"]

        result = V4Backtester(
            self._config(strategy, starting_capital)
        ).run(bars, execution_features, predictions)
